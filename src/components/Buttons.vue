...
export default {
  setup() {
    ...
    function removeButton(id: number) {
      ...
      buttons.value.list.splice(index, 1)
      saveData()
    }
    ...
    function btnMouseMove(e){ 
      ...
      (e.pageY + currentBtn.offset.bottom < (containerPos.y + containerHeight)) ){
        ...
        currentBtn.el.path[1].style.zIndex = buttons.value.lastZindex + 1
    ...
    function btnMouseResizeMove(e){ 
      e = fixEvent(e)
      for ( let i = 0; i < table.value.cells.length; i++ ) {
        const targ = table.value.cells[i]
        ...
        (e.pageY < (targPos.y + targHeight)) ){
          ...
          if ( btn.xClickedPosition == 1 || btn.xClickedPosition == btn.height ) {
            const hDif: number = (table.value.model.findIndex(arr => arr.includes(containerIndex)) - top) - table.value.model.findIndex(arr => arr.includes(i))
          ...
          if ( btn.yClickedPosition == 1 || btn.yClickedPosition == btn.width ) {
            const wDif: number = table.value.model[table.value.model.findIndex(arr => arr.includes(containerIndex))].findIndex(x => x == containerIndex) -
                        table.value.model[table.value.model.findIndex(arr => arr.includes(i))].findIndex(x => x == i)
    ...
    function generateButtonId() { return buttons.value.lastId++ }
    function generateButtonZindex() { return buttons.value.lastZindex++ }
    function getButton(id: number) { 
      const index: number = buttons.value.list.findIndex((btn) => btn.id == id)
      const btn: object = buttons.value.list[index]
      const containerIndex: number = buttons.value.list[index].containerIndex
    ...
    function changeBtnContainerForResize() {
      ...
      if ( btn.height > 1 ) {
        if ( xClickedPosition == 1 ) {
          btn.containerIndex += table.value.cols * btn.directions.bottom
          btn.directions.top += btn.directions.bottom
          btn.directions.bottom = 0
        }
        if ( xClickedPosition == btn.height ) {
          btn.containerIndex -= table.value.cols * btn.directions.top
    ...
    function checkButtonOnDirections(id: number) {
      ...
      commonCells.top = table.value.model.findIndex(arr => arr.includes(containerIndex))
      commonCells.bottom = table.value.model.length - commonCells.top - 1
      commonCells.left = table.value.model[table.value.model.findIndex(arr => arr.includes(containerIndex))].findIndex(x => x == containerIndex)
      commonCells.right = table.value.model[table.value.model.findIndex(arr => arr.includes(containerIndex))].length - commonCells.left - 1
    ...
    function checkButtonsOnChangeTable() {
      buttons.value.list.forEach(btn => {
        if ( btn.containerIndex < (table.value.cols * table.value.rows) ) checkButtonOnDirections(btn.id)
        btn.hidden = btn.width > table.value.cols || btn.height > table.value.rows
      })
    }
  ...
  methods: {
    loadDataFromStorage: function() {
      ...
      this.buttons.list = data.buttons.list
      this.buttons.lastId = data.buttons.lastId
      this.buttons.lastZindex = data.buttons.lastZindex
    },
    fillCells: function() {
      this.table.model = this.createTableModel()
      this.table.cells = this.$refs.container.querySelectorAll('.cell')
      this.checkButtonsOnChangeTable()
    },
